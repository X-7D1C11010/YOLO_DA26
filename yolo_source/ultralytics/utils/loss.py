...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk: int = 10, tal_topk2: int | None = None):  # model must be de-paralleled
        ...
        self.domain_weight = getattr(h, 'domain_weight', 0.1) if h else 0.1
    ...
    def __call__(
        ...
        # =========================================================
        # 🔥 2. 真·有监督核心：取消所有隔离，让模型“看”所有标签！
        # =========================================================
        # 🚨 重点在这里：我们直接把包含 SAR 真实标签的原始 batch 送了进去！
        # 这意味着检测器将直接利用 SAR 的真实框来更新自己的权重，变成有监督学习。
        loss_sum, loss_items = self.loss(self.parse_output(normal_preds), batch)
        ...
        if domain_preds is not None and self.domain_weight > 0:
            ...
            # 【新增】域对抗损失权重（可调节）
            domain_weight = getattr(self, 'domain_weight', 0.1)
