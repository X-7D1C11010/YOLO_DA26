...
class BaseModel(torch.nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        Returns:
            (torch.Tensor | tuple): The last output of the model. If training and DomainClassifier
                                    is present, returns (detection_output, domain_output).
        """
        y, dt, embeddings = [], [], []  # outputs
        domain_output = None  # 【新增】：用于保存域判别器的结果
        alpha = getattr(self, 'da_alpha', 1.0)  # 【新增】：动态获取对抗梯度反转系数
        ...
        # 【新增】：在训练模式下，如果我们截获了域判别结果，就把它一起打包带走
        if self.training and domain_output is not None:
            if isinstance(x, dict):
                # 如果原版输出已经是字典，我们直接加一个键值对进去，完美隐身！
                x['domain_preds'] = domain_output
                return x
            else:
                # 兼容旧版本
